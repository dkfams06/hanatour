...
export async function POST(
  ...
  try {
    ...
    // 현재 상태가 승인 또는 거부 상태인지 확인
    if (review.status === 'pending') {
      return NextResponse.json(
        { success: false, error: '이미 대기 상태인 후기입니다.' },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json({
      success: true,
      message: '후기 상태가 대기 상태로 되돌려졌습니다.'
    });

  } catch (error) {
    console.error('후기 상태 되돌리기 에러:', error);
    return NextResponse.json(
      { success: false, error: '후기 상태 되돌리기 중 오류가 발생했습니다.' },
      { status: 500 }
    );
  }
}
