...
export async function POST(request: Request) {
  try {
    const { username, nickname, email, password, name, phone, birthDate, address } = await request.json();
    ...
    await pool.query(
      'INSERT INTO users (id, username, nickname, email, password, name, phone, birthDate, address, role, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
      [id, username, nickname, email, hashed, name, phone, birthDate, address, 'customer', 'approved']
    );
